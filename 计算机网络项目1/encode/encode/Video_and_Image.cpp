...
bool Video_and_Image::image_to_vedio_fun()
{
	...
	int number_of_image = Create_images::get_number_of_imgae() + 1;
	double time_per_frame =  1000.0*number_of_image/(double)video_length ;
	video.open(image_to_video_des, CV_FOURCC('X', 'V', 'I', 'D'), time_per_frame, Size(1080, 1080), true);// 构造一个VideoWriter(地址、格式、帧率、大小、灰度值)
	cout <<"count="<< count << endl;
	cout << "number_of_image=" << number_of_image << endl;
	//int number_of_image = Create_images::get_number_of_imgae() + 1;
	for (size_t i = 0; i < number_of_image; i++)
	{
		//cout << fn[i] << endl;
